...
class vdos:
    ...
    def prep(self):
        ...
        error = vdosLib.allocResidueList(
            ct.pointer(residueList), ct.c_int(self.nRes), ct.c_int(self.nCorr)
        )
        MDinfo = t_MDinfo()
        error = vdosLib.allocMDinfo(
            ct.pointer(MDinfo), ct.c_int(self.nCorr), ct.c_int(self.sel.n_atoms)
        )
        r = 0
        for res in self.sel.residues:
            ...
            r += 1
        vdosLib.setArrayIndexOffsets(
            ct.pointer(residueList), ct.pointer(MDinfo)
        )
        ...
        for res in self.sel.residues:
            ...
            r += 1
        vdosLib.setArrayIndexOffsets(
            ct.pointer(residueListCopy), ct.pointer(MDinfo)
        )
        error = vdosLib.getRotBonds(
            ct.pointer(residueListCopy),
            ct.c_int(self.nRes),
    ...
    def postProcess(self, residueList, mode="all", resIdx=0):
        ...
        if residueList.postProcessed == 0:
            ...
            if mode == "single":
                vdosLib.normResData(
                    ct.pointer(residueList.residues[resIdx]),
                    ct.c_int(self.nCorr),
                )
                ...
                for i in range(3):
                    self.trVACF[0][i] = residueList.trCorr[
                        i : 3 * self.nCorr : 3
                    ]
                    self.symFT(self.trVACF[0][i], self.trVDoS[0][i])
                    self.rotVACF[0][i] = residueList.rotCorr[
                        i : 3 * self.nCorr : 3
                    ]
            ...
            if mode == "all":
                for i in range(1, self.nRes + 1):
                    self.totVACF[i] = residueList.residues[i - 1].totCorr[
                        0 : self.nCorr
                    ]
                    self.symFT(self.totVACF[i], self.totVDoS[i])
                    for j in range(3):
                        self.trVACF[i][j] = residueList.residues[i - 1].trCorr[
                            j : 3 * self.nCorr : 3
                        ]
                        self.symFT(self.trVACF[i][j], self.trVDoS[i][j])
                        self.rotVACF[i][j] = residueList.residues[
                            i - 1
                        ].rotCorr[j : 3 * self.nCorr : 3]
                        self.symFT(self.rotVACF[i][j], self.rotVDoS[i][j])
                    self.rotBondVACF[i] = residueList.residues[
                        i - 1
                    ].rotBondCorr[0 : self.nCorr]
                ...
                if mode == "single":
                    ...
                    i = 1
                self.totVACF[i] = residueList.residues[resIdx].totCorr[
                    0 : self.nCorr
                ]
                self.symFT(self.totVACF[i], self.totVDoS[i])
                for j in range(3):
                    self.trVACF[i][j] = residueList.residues[resIdx].trCorr[
                        j : 3 * self.nCorr : 3
                    ]
                    self.symFT(self.trVACF[i][j], self.trVDoS[i][j])
                    self.rotVACF[i][j] = residueList.residues[resIdx].rotCorr[
                        j : 3 * self.nCorr : 3
                    ]
                    self.symFT(self.rotVACF[i][j], self.rotVDoS[i][j])
                self.rotBondVACF[i] = residueList.residues[resIdx].rotBondCorr[
                    0 : self.nCorr
                ]
                self.symFT(self.rotBondVACF[i], self.rotBondVDoS[i])
    ...
    def outputGeometry(self, outputFileName, residueList):
        ...
        if averRotBondCount > 0:
            ...
            averLogRotBondInertia = (
                residueList.logRotBondInertia / averRotBondCount
            )
        else:
        ...
        outFile.write(
            "#Average Rotatable Bond Inertia (g/mol*A^2): (%d)\n%20.6e\n"
            % (averRotBondCount, averRotBondInertia)
        )
        outFile.write(
            "#Average Rotatable Bond Log(Inertia): (%d)\n%20.6e\n"
    ...
    def outputVACF(self, outputFileName):
        ...
        for i in range(self.nCorr):
            outFile.write(
                "%-20.6e %-20.6e %-20.6e %-20.6e %-20.6e %-20.6e %-20.6e %-20.6e %-20.6e\n"
                % (
                    self.tau[i],
                    self.trVACF[0][0][i],
                    self.trVACF[0][1][i],
